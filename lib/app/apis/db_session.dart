...
class DbSession {
  Future<Faculty> getFacultybyUserId(Session session,StreamProviderRef ref) async {
    return   ref.read(dbProvider).doc(ref.read(institutionNotifierProvider).value!.docRef!.path).collection('faculty')
 .where('userId',isEqualTo: session.faculty!.userId).get().then((snapshot) => Faculty.fromMap(snapshot.docs[0].data()));
